...
void Map::Render(ShaderProgram* program)
{
	...
	/*
	If code crashes and brings you here please try again.
	Sometimes it takes multiple tries to work so please keep trying. Sorry and thank you.
	Maybe it crashes when switching levels too fast so please try again and kill enemies slowly.
	*/
...
bool Map::IsSolid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
	...
	*penetration_y = 0;
	...
	if (11 <= tile && tile <= 14) return false;
	else if (21 <= tile && tile <= 24) return false;
	else if (31 <= tile && tile <= 34) return false;
	else if (6 <= tile && tile <= 9) return false;
	else if (16 <= tile && tile <= 19) return false;
	else if (26 <= tile && tile <= 29) return false;
	else if (60 <= tile && tile <= 63) return false;
	else if (69 <= tile && tile <= 73) return false;
	else if (tile == 78) return false;
	else if (tile == 79) return false;
